...
!function($) {
	...
	var Datepicker = function(element, options) {
		...
		this.picker = $(DPGlobal.template).appendTo('body').hide().on(
				'mousedown.Datepicker', $.proxy(this.mousedown, this)).on(
	...
	Datepicker.prototype = {
		...
		show : function(e) {
			$('div.datepicker.dropdown-menu').hide(); // make sure to hide all
			// other calendars
			this.picker.show();
			...
			if (!this.isInput) {
				$(document)
						.on('mousedown.Datepicker', $.proxy(this.hide, this));
			}
		...
		update : function() {
			...
			this.date = DPGlobal.parseDate(date ? date : this.element
					.data('date'), this.format);
	...
	var DPGlobal = {
		...
		parseFormat : function(format) {
			var separator = format.match(/[.\/-].*?/), parts = format
					.split(/\W+/);
		...
		parseDate : function(date, format) {
			...
			if (!date)
				date = "";
			var parts = date.split(format.separator), date = new Date(today
					.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0), val;
