...
/**
 * 
 * @ClassName IndexOncampusCharge
 * @Description 校内收费	统计时段内，该仪器的使用者为校内人员的使用收费总和
 * @author hujinzhe
 * @date 2014年8月25日 下午15:53
 *
 */
@Component("on_cam_charge")
public class IndexOncampusCharge extends IndexBase {
	...
	public void run(long startDate, long endDate,
		...
		for (Map<String, Object> result : results) {
			...
			if (equipments.containsKey(key)) {
				...
				equipmentIndexEntity.setOnCamCharge(equipmentIndexEntity
						.getOnCamCharge() + amount.doubleValue());
			} else {
				EquipmentIndexEntity equipmentIndexEntity = new EquipmentIndexEntity(
						getId(), iEquipmentId, sUserId, startDate);
				equipmentIndexEntity.setOnCamCharge(amount.doubleValue());
