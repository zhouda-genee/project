 ...
 *
 */
@Component("serv_scientific_cnt")
public class IndexServiceScientificCount extends IndexBase {
	...
	public void run(long startDate, long endDate,
		...
		for (Map<String, Object> result : results) {
			...
			if (equipments.containsKey(key)) {
				...
				equipmentIndexEntity.setServScientificCnt(equipmentIndexEntity
						.getServScientificCnt() + count);
			} else {
				EquipmentIndexEntity equipmentIndexEntity = new EquipmentIndexEntity(
						getId(), iEquipmentId, sUserId, startDate);
				equipmentIndexEntity.setServScientificCnt(count);
	...
	private List<Map<String, Object>> queryResult(long startDate, long endDate) {
		String sql = "select t.userid, t.equipmentid, sum(t.procount) as procount "
				...
				+ "where rec.reserv_id is null and pro.type = 1 and res.dtstart between ? and ? "
				...
				+ "where pro.type = 1 and rec.dtstart between ? and ? "
				...
				+ "where pro.type = 1 and sam.dtstart between ? and ? "
