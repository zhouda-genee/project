 ...
 *
 */
@Component("serv_society_cnt")
public class IndexServiceSocietyCount extends IndexBase {
	...
	public void run(long startDate, long endDate,
		...
		for (Map<String, Object> result : results) {
			...
			if (equipments.containsKey(key)) {
				...
				equipmentIndexEntity.setServSocietyCnt(equipmentIndexEntity
						.getServSocietyCnt() + count);
			} else {
				EquipmentIndexEntity equipmentIndexEntity = new EquipmentIndexEntity(
						getId(), iEquipmentId, sUserId, startDate);
				equipmentIndexEntity.setServSocietyCnt(count);
	...
	private List<Map<String, Object>> queryResult(long startDate, long endDate) {
		String sql = "select t.userid, t.equipmentid, sum(t.procount) as procount "
				...
				+ "where rec.reserv_id is null and pro.type = 2 and res.dtstart between ? and ? "
				...
				+ "where pro.type = 2 and rec.dtstart between ? and ? "
				...
				+ "where pro.type = 2 and sam.dtstart between ? and ? "
